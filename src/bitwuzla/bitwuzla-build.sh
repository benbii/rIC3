...
if ! test -f .libs/libgmp.a; then
  CC=clang AR=llvm-ar RANLIB=llvm-ranlib CFLAGS=-flto ./configure --enable-shared=no
  make -j"$jobs"
fi

cd ../mpfr-4.2.2
if ! test -f src/.libs/libmpfr.a; then
  CC=clang AR=llvm-ar RANLIB=llvm-ranlib ./configure --enable-lto \
    --disable-shared --with-gmp-build=../gmp-6.3.0
  make -j"$jobs"
fi
