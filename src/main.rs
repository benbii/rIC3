...
use rIC3::{
    ...
    transys::{self, Transys, certify::Restore},
    wlbmc::WlBMC, wlkind::WlKind,
};
use std::{
    env, fs, fs::File, io::BufWriter, io::BufReader, path::{Path, PathBuf}, process::ExitCode,
...
fn main() -> ExitCode {
    ...
    match Commands::parse() {
        ...
        Commands::Preprocess { pp } => {
            let ts = load(&pp.model);
